...
public class _03_ConsumerSupplierDemos {
    ...
    private static void persistAccount(Account account) {
        ...
        saveAccountTo(account, toFile); // guarda en archivo
        saveAccountTo(account, toDB); // guarda en base de datos
        saveAccountTo(account, acnt -> saveToFile(acnt)); // guarda en archivo, aqui no se creo el consumer toFile
        saveAccountTo(account, acnt -> getDataBaseConnection().insert(acnt)); // guarda en base de datos, aqui no se creo el consumer toDB
    }
...
//
    ...
    private static void saveToFile(Account account) {
        // Simalacion de guardar en archivo
        System.out.println("Guardando en archivo: " + account);
    }

    private static DataBaseExecutor getDataBaseConnection() {
        // Simulacion de conexion a base de datos
    ...
    private interface DataBaseExecutor {
        <T> void insert(T instance); // Almacena un objeto en la base de datos

        <T> T select(String id); // Busca un objeto en la base de datos
    }
}
